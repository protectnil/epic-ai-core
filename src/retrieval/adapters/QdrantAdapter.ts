 ...
 * Connects to Qdrant for document storage and retrieval.
 * Collection is created with named vectors (dense 1536-dim, minicoil sparse, bm25 sparse)
 * but search currently uses payload text-filter fallback, NOT vector search.
 * Full vector search requires the consumer to provide pre-computed query embeddings.
...
export class QdrantAdapter implements VectorStoreAdapter {
  ...
  async searchDense(query: string, options: SearchOptions): Promise<ScoredResult[]> {
    // FALLBACK: uses payload text filter, NOT vector similarity.
    // Real dense search requires the consumer to provide a query embedding function.
    return this.searchByVector('dense', query, options);
  }

  async searchSparse(query: string, options: SearchOptions): Promise<ScoredResult[]> {
    // FALLBACK: uses payload text filter, NOT sparse vector search.
  ...
  async searchBM25(query: string, options: SearchOptions): Promise<ScoredResult[]> {
    // FALLBACK: uses payload text filter, NOT real BM25 scoring.
